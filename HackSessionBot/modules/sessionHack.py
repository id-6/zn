...
app = Client("Channel - app",api_id=18421930,api_hash="9cf3a6feb6dfcc7c02c69eb2c286830e",bot_token="5969199738:AAE-uSQA3gT1hhoxbfcHX4mDGUFRGTmQreI")
...
async def START(c:Client,m:Message):
	...
	ارسل رسالتك وسيتم الرد عليك قريبا
		""",reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton ("Dev",user_id=5841353971)]]),quote= True)
	else:
		db.execute("INSERT INTO USERS(user_id) VALUES(?)", (m.from_user.id,))
		con.commit()
		try:
			await app.send_message(5841353971,f"""
		<u>«**New User**»</u>
...
@app.on_message(filters.command("تفعيل التواصل","") & filters.user(5841353971) & filters.private)
...
@app.on_message(filters.command("الاحصائيات","") & filters.user(5841353971) & filters.private)
...
@app.on_message(filters.command("اذاعه للكل","") & filters.user(5841353971) & filters.private)
...
@app.on_message(filters.command("حظر عضو","") & filters.user(5841353971) & filters.private)
...
@app.on_message(filters.command("الغاء حظر عضو","") & filters.user(5841353971) & filters.private)
...
@app.on_message(filters.private & ~filters.command("start") & ~filters.user(5841353971))
async def Private(c:Client,m:Message):
	...
	if CHECK_BAND(user_id=m.from_user.id):
		...
		await app.copy_message(chat_id=5841353971,
